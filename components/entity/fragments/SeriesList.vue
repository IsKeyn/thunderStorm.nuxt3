...
<template>
	<div class="mb-6">
		...
		>
			<template #default>
				...
				>
					...
					>
						...
						<SliderWithSlots
								:count="preparationData(element[dataKey]).length"
								:carouselConfig="carouselConfig"
								:usePagination="false"
						>
							<template
									v-for="(item, key) in preparationData(element[dataKey])"
									:key="key"
			...
			<template v-if="data.length > 1" #hiddenContent >
				<template
						v-for="(element, key) in data"
						:key="key"
				>
