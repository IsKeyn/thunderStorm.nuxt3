...
watch(() => props.additionalData, (newValue) => {
	const key = 'gaming_platform';

	if (newValue && newValue[key]) {
		toRaw(newValue[key]).forEach((item) => {
			repeaterItem[key].options.push({
				name: item.name,
				value: item.id,
			});
		});
		...
		>
			...
			<div class="buttons-box">
				<button
						...
						class="btn"
						@click="deleteRepeatorItem(index)"
				>
