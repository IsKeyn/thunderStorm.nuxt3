...
<template>
	<nuxt-link
			:to="`/game/${element.slug}`"
			target="_blank"
			:class="['item-box', classes]"
			:style="`background-image: url('${getResizeImg(element.title_image, 1500)}');`"
	>
		...
		<div class="content-box">
			<div class="info">
				...
				<div class="genres" v-if="element.genres.length">
					<span
							v-for="(item, key) in element.genres"
							:key="key"
					>
						{{ item.name }} <span v-if="key !== element.genres.length - 1">| </span>
					</span>
				</div>
			</div>
		</div>
...
.item-box {
	...
	.content-box {
		@apply absolute top-0 left-0 z-[1] w-full h-full flex items-center justify-center;
	}
