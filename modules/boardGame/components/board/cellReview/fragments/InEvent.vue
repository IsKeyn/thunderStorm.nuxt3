...
const props = defineProps({
	...
	contentBlockClass: {
		type: String,
		default: 'in-event-cell-list-content',
	},
	element: {
		type: Object,
		default: {},
...
const requestName = 'player_reviews_in_event_' + props.eventSlug + '_' + props.element.boardPositionEffect.id;
		...
		async () => {
			if (props.eventSlug && props.element.boardPositionEffect.id) {
				...
				const response = await Promise.resolve(
						sendApiRequest(
								`board-game/v2/board-cell/get-current-event-review/${props.eventSlug}/${props.element.boardPositionEffect.id}`,
								'GET',
								query,
								requestName,
								''
...
<template>
	...
	<ui-itemBox
			v-else
			classes="red"
			message="Участники ещё не оставили отзывы о данном ячейке игрового поля"
	/>
