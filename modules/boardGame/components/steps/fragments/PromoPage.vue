...
<template>
	<template v-if="boardGameStore.boardGameInfo">
		...
		<div class="add-info-box">
			...
			Вы также можете посмотреть <nuxt-link :to="'/e/' + route.params.slug + '/game?tab=game-list'" target="_blank" title="Игры ивента">игры</nuxt-link>, которые участвуют в ивенте, посмотреть <nuxt-link :to="'/e/' + route.params.slug + '/inventory?tab=item-list'" target="_blank" title="Предметы ивента">предметы</nuxt-link>, которые можно будет использовать, а также ознакомиться с полными <nuxt-link :to="'/e/' + route.params.slug + '/rules'" target="_blank" title="Правила ивента">правилами</nuxt-link> ивента.
		</div>
	</template>
</template>
